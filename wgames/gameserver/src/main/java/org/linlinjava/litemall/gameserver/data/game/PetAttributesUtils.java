...
//    }
     ...
     *
     * @param quality           品质 1普通  1精怪  2变异  3神兽
     * @param use_attrib        携带等级
     * @param currentStep     宠物的当前阶段  初级1  中级2  高级3  完成羽化
    ...
    public static int[] emergencePet(int quality, int use_attrib, int currentStep, int currentReiki, int pill, int equipmentMoney, int UnidentifiedMoney, int[] appends) {
        ...
        //如果当前灵气大于最大值的95%就成功
        if (newReiki >= (int)maxReiki * 0.95) {
            //完成
            result[0] = 1;
            result[1] = 0;
        } else {
            if (newReiki > (int)maxReiki * 0.7 && new Random().nextInt(10) < 2) {
                result[0] = 1;
                result[1] = 0;
            } else {
            	result[0] = 0;
