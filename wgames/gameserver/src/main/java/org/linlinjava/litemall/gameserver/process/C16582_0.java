        ...
        if (type == 2) {
            ...
            for (int i = 0; i < chara.pets.size(); i++) {
                if (chara.pets.get(i).id == inventoryPos) {
                    ...
                    SaleClassifyGood saleClassifyGood = GameData.that.baseSaleClassifyGoodService.findOneByStr(pet.petShuXing.get(0).str);         
                    if (saleClassifyGood == null) {
                        return;
                    }
                    if(pet.petShuXing.get(0).penetrate==4) {
                    	Vo_20480_0 vo_20480_0 = new Vo_20480_0();
                        vo_20480_0.msg = "神兽不能在此出售";
                        vo_20480_0.time = (int) System.currentTimeMillis() / 1000;
                        GameObjectChar.send(new M20480_0(), vo_20480_0);
                    	return;
                    }
