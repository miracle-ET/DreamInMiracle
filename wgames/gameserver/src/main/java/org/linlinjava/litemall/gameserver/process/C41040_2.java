...
public class C41040_2 implements GameHandler {
    ...
    public void process(ChannelHandlerContext ctx, ByteBuf buff) {
        ...
        if(!GameUtil.removemunber(chara, "控心玉", 1)) {
        	return;
        }
        if(!GameUtil.removemunber(chara, "定鞍石", 1)) {
        	return;
        }
        if(!GameUtil.removemunber(chara, "驱力刺", 1)) {
        	return;
        }
        if(!GameUtil.removemunber(chara, "困灵砂", 1)) {
        	return;
        }
        if(!GameUtil.removemunber(chara, "拘首环", 1)) {
        	return;
        }
