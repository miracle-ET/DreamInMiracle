...
//        改名字
        if (type == 1) {
            ...
            if(!GameUtil.removemunber(chara, "改头换面卡", 1)) {
            	return;
            }
        ...
        if (4 == type) {
            for (int i = 0; i < chara.pets.size(); i++) {
                if (chara.pets.get(i).no == Integer.valueOf(para1)) {
                    ...
                    int mana = 0;
                    int accurate = 0;
                    for (PetShuXing info : petShuXing) {
                    	//找到para2妖石对应的属性记录并移除
						if(info.str.equals(para2)) {
                            wiz = info.wiz;
                            parry = info.parry;
                            def = info.def;
                            dex = info.dex;
                            ...
                            petShuXing.remove(info);
                            break;
						}
					}
                    //刷新总属性
                    PetShuXing info = petShuXing.get(0);
                    info.wiz -= wiz;
                    info.parry -= parry;
                    info.def -= def;
                    info.dex -= dex;
                    info.mana -= mana;
                    info.accurate -= accurate;
