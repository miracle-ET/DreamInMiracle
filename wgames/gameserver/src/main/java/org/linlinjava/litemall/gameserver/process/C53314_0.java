        ...
        if (type.equals("pet_open_eclosion")) {
            for (int i = 0; i < chara.pets.size(); i++) {
                ...
                if (petbeibao.no == no) {
                    ...
                    if(!GameUtil.removemunber(chara, "羽化丹", 1)) {
                    	return;
                    }
            ...
            for (int i = 0; i < chara.pets.size(); i++) {
                ...
                if (petbeibao.no == no) {
                    ...
                    if (ints[0] == 1) {
                        ...
                        petbeibao.petShuXing.get(0).pet_mana_shape = petbeibao.petShuXing.get(0).pet_mana_shape + ints[2] / 4;
                        petbeibao.petShuXing.get(0).pet_speed_shape = petbeibao.petShuXing.get(0).pet_speed_shape + ints[3] / 4;
                        petbeibao.petShuXing.get(0).pet_mag_shape = petbeibao.petShuXing.get(0).pet_mag_shape + ints[4] / 3;
                        petbeibao.petShuXing.get(0).rank = petbeibao.petShuXing.get(0).rank + ints[5] / 4;
                    ...
                    list.add(petbeibao);
                    GameObjectChar.send(new M65507_0(), list);
                    if(!GameUtil.removemunber(chara, "羽化丹", pill)) {
                    	return;
                    }
...
//                    public int phy_absorb=65504;
                    ...
                    list.add(petbeibao);
                    GameObjectChar.send(new M65507_0(), list);
                    if(!GameUtil.removemunber(chara, "点化丹", pill)) {
                    	return;
                    }
...
//  //原始的气血 法力 物伤 法伤 速度
                    ...
                    list.add(petbeibao);
                    GameObjectChar.send(new M65507_0(), list);
                    if(!GameUtil.removemunber(chara, "点化丹", 1)) {
                    	return;
                    }
